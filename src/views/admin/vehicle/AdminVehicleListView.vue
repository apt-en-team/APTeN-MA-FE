...
const openSuccessModal  = (message) => { successModal.message = message; successModal.show = true }
const closeSuccessModal = () => { successModal.show = false; successModal.message = '' }
...
const openDetailModal = async (vehicle) => {
  ...
  if (vehicle?.household?.dong) {
    try {
      const { data } = await vehicleAPI.getAllVehicles({ dong: vehicle.household.dong, size: 999, status: null })
      const sameHouseholdVehicles = data.resultData?.content?.filter(v => v.household?.ho === vehicle.household.ho) ?? []
      vehicleCount.value = sameHouseholdVehicles.length
    } catch (e) { console.error('차량 개수 조회 실패', e); vehicleCount.value = 0 }
  } else { vehicleCount.value = 0 }
}
...
const openRejectModal = (vehicle) => {
  rejectModal.vehicle = vehicle; rejectModal.loading = false
  rejectModal.reason  = ''; rejectModal.memo = ''; rejectModal.show = true
}
...
const openDeleteModal  = (vehicle) => { deleteModal.vehicle = vehicle; deleteModal.loading = false; deleteModal.show = true }
const closeDeleteModal = () => { deleteModal.show = false; deleteModal.vehicle = null }
...
const openRegisterModal = async () => {
  registerForm.dong = ''; registerForm.householdId = null; registerForm.userId = null
  registerForm.licensePlate = ''; registerForm.carModel = ''; registerForm.carType = ''
  errors.userId = ''; errors.licensePlate = ''; errors.carModel = ''; errors.carType = ''; errors.general = ''
  registerHouseholds.value = []; registerResidents.value = []; vehicleCount.value = 0
...
const closeRegisterModal = () => { registerModal.show = false }

const onDongChange = async () => {
  registerForm.householdId = null; registerForm.userId = null
  registerResidents.value  = []; vehicleCount.value = 0

  if (!registerForm.dong) { registerHouseholds.value = []; return }
...
const onHouseholdChange = async () => {
  registerForm.userId = null; vehicleCount.value = 0

  if (!registerForm.householdId) { registerResidents.value = []; return }
  ...
  } catch (e) { console.error('입주민/차량 조회 실패', e) }
}

const handleRegister = async () => {
  errors.userId = ''; errors.licensePlate = ''; errors.carModel = ''; errors.carType = ''; errors.general = ''

  if (!registerForm.userId)       { errors.userId       = '입주민을 선택해주세요.'; return }
  if (!registerForm.licensePlate) { errors.licensePlate = '차량 번호를 입력해주세요.'; return }
  if (!registerForm.carModel)     { errors.carModel     = '차 모델을 입력해주세요.'; return }
  if (!registerForm.carType)      { errors.carType      = '차종을 선택해주세요.'; return }
  if (vehicleCount.value >= 2)    { errors.general      = '세대당 최대 2대까지 등록 가능합니다.'; return }

  try {
    const { data } = await vehicleAPI.getAllVehicles({ search: registerForm.licensePlate, size: 999 })
    const existingVehicle = data.resultData?.content?.find(
      v => v.licensePlate === registerForm.licensePlate && (v.status === 'APPROVED' || v.status === 'PENDING')
    )
    if (existingVehicle) { errors.licensePlate = '이미 등록된 차량입니다. (승인 또는 대기 중)'; return }
  } catch (e) { console.error('중복 체크 실패:', e) }

  registerModal.loading = true
  try {
    await vehicleAPI.adminRegisterVehicle({
      userId: registerForm.userId, licensePlate: registerForm.licensePlate,
      carModel: registerForm.carModel, carType: registerForm.carType, status: 'APPROVED',
    })
    closeRegisterModal()
    ...
    if (errorMsg.includes('차량') || errorMsg.includes('번호')) { errors.licensePlate = errorMsg }
    else { errors.general = errorMsg }
  } finally { registerModal.loading = false }
}

watch(() => route.query.register, (val) => { if (val) openRegisterModal() })

const doSearch = () => { state.currentPage = 1; fetchVehicles() }

let searchTimer = null
const onSearch = () => { clearTimeout(searchTimer); searchTimer = setTimeout(() => doSearch(), 300) }

const resetFilters = () => {
  state.searchQuery = ''; state.filterStatus = ''; state.filterHousehold = ''
  state.filterType  = ''; state.filterDong   = ''; state.currentPage     = 1
  fetchVehicles()
}
...
onMounted(() => {
  fetchVehicles(); fetchStats(); fetchDongs()
  ...
  <div class="vehicle-list-view">
    ...
    <BaseModal v-if="detailModal.show" title="차량 상세 정보" :subtitle="'ID ' + detailModal.vehicle?.vehicleId" @close="closeDetailModal">
      <div class="detail-header-card">
        ...
        <p class="detail-header-sub">{{ detailModal.vehicle?.carModel ?? '-' }} · {{ householdLabel(detailModal.vehicle?.household) }}</p>
      </div>
      ...
      <div class="detail-grid">
        ...
        <div class="detail-cell">
          ...
          <span class="detail-value">{{ householdLabel(detailModal.vehicle?.household) }}</span>
        </div>
        <div class="detail-cell">
        ...
        <div class="detail-cell">
          ...
          <span class="detail-value">{{ formatDate(detailModal.vehicle?.createdAt) }}</span>
        </div>
        <div class="detail-cell">
      ...
      <template #footer>
        <button class="btn-delete-vehicle" @click="openDeleteModal(detailModal.vehicle)">차량 삭제</button>
        <button class="btn-cancel" @click="closeDetailModal">닫기</button>
      </template>
    </BaseModal>

    <!-- 차량 삭제 확인 모달 -->
    <BaseModal v-if="deleteModal.show" title="차량을 삭제하시겠습니까?" @close="closeDeleteModal">
      <div class="confirm-vehicle-card">
        ...
        <p class="confirm-vehicle-sub">{{ deleteModal.vehicle?.carModel ?? '-' }} · {{ householdLabel(deleteModal.vehicle?.household) }}</p>
      </div>
      <p class="confirm-hint">삭제 후 복구가 불가능합니다.</p>
      <template #footer>
        <button class="btn-cancel" @click="closeDeleteModal">취소</button>
        <button class="btn-submit-reject" @click="handleDelete" :disabled="deleteModal.loading">
    ...
    <BaseModal v-if="rejectModal.show" title="차량 거부" :subtitle="'ID ' + rejectModal.vehicle?.vehicleId" @close="closeRejectModal">
      ...
      <template #footer>
        ...
        <button class="btn-submit-reject" @click="handleReject" :disabled="rejectModal.loading || !rejectModal.reason">
    ...
    <BaseModal v-if="registerModal.show" title="차량 등록" subtitle="새 차량 정보를 입력해주세요" @close="closeRegisterModal">
      ...
      <div class="form-group">
        ...
        <select v-model="registerForm.userId" class="form-select" :class="{ 'input-error': errors.userId }" :disabled="!registerForm.householdId">
      ...
      <div class="form-group">
        ...
        <input v-model="registerForm.licensePlate" class="form-input" :class="{ 'input-error': errors.licensePlate }" placeholder="예: 12가 3456" />
      ...
      <div class="form-row-2">
        <div class="form-group">
          ...
          <input v-model="registerForm.carModel" class="form-input" :class="{ 'input-error': errors.carModel }" placeholder="예: 현대 소나타" />
          <span v-if="errors.carModel" class="field-error">{{ errors.carModel }}</span>
        </div>
        <div class="form-group">
          <label class="form-label">차종 <span class="required">*</span></label>
          <select v-model="registerForm.carType" class="form-select" :class="{ 'input-error': errors.carType }">
      ...
      <template #footer>
        ...
        <button type="button" class="btn-submit-approve" @click="handleRegister" :disabled="registerModal.loading || vehicleCount >= 2">
...
.td-dash { color: #A0AEC0; font-size: 14px; }
...
/* 상세 모달 헤더 카드 */
.detail-header-card  { background: #F7FAFC; border-radius: 12px; padding: 20px; margin-bottom: 24px; }
.detail-header-badge { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 600; margin-bottom: 10px; }
.detail-header-badge.approved { background: #C6F6D5; color: #276749; }
.detail-header-badge.pending  { background: #FEFCE8; color: #C08B2D; }
.detail-header-badge.rejected { background: #FFF5F5; color: #E53E3E; }
.detail-header-title { font-size: 22px; font-weight: 700; color: #1A202C; margin-bottom: 6px; }
.detail-header-sub   { font-size: 13px; color: #718096; }

/* 상세 그리드 */
.detail-grid  { display: grid; grid-template-columns: 1fr 1fr; gap: 0; margin-bottom: 8px; }
.detail-cell  { display: flex; flex-direction: column; gap: 4px; padding: 14px 16px; border-bottom: 1px solid #F0F4F8; }
.detail-cell:nth-last-child(-n+2) { border-bottom: none; }
.detail-label { font-size: 11px; color: #A0AEC0; }
.detail-value { font-size: 13px; color: #1A202C; font-weight: 500; }
.detail-value.bold { font-weight: 700; }

/* 삭제 버튼 */
.btn-delete-vehicle { padding: 8px 10px; background: #FFFFFF; color: #E53E3E; border: 1px solid #E53E3E; border-radius: 7px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: 'Noto Sans KR', sans-serif; margin-right: auto; }
.btn-delete-vehicle:hover { background: #E53E3E; color: #FFFFFF; }
