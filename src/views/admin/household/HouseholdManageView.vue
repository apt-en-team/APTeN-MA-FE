...
const getMaxPage = async () => {
  try {
    const result = await householdAPI.getMaxPage(
      state.size,
      state.dong,
      state.ho,
      state.status
  ...
  <div class="household-page">
    ...
    <div class="table-section">
      <FilterBar @reset="resetFilters">
        ...
        <select class="filter-select" v-model="state.dong" @change="doSearch">
          ...
          <option v-for="dong in state.dongOptions" :key="dong" :value="dong">
            {{ dong }}
          </option>
        </select>
      ...
      <AdminTable :columns="columns" :rows="state.list" @row-click="openDetailModal">
        ...
        <template #action="{ row }">
          <button
            ...
            class="btn-register"
            @click.stop="openConfirmModal($event, row)"
          >
            승인요청
          </button>
    ...
    >
      <div class="detail-hero">
        ...
        <h2 class="detail-title">
          {{ detailModal.item?.dong }} {{ detailModal.item?.ho }}
        </h2>
      ...
      <div v-else-if="detailModal.residents.length === 0" class="detail-empty">
        등록된 입주민이 없습니다.
      </div>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input
            class="form-input"
            :value="
              editModal.item?.carCount != null ? editModal.item.carCount + '대' : '-'
            "
            disabled
          />
        </div>
    ...
    <Modal v-if="confirmModal.show" title="승인 처리" @close="closeConfirmModal">
      <p class="confirm-message">
        {{ confirmModal.item?.dong }} {{ confirmModal.item?.ho }} 세대<br />승인 대기 중인
        회원 {{ confirmModal.users.length }}명
      </p>

      <div v-if="confirmModal.loading" class="detail-empty">조회 중...</div>
      <div v-else-if="confirmModal.users.length === 0" class="detail-empty">
        대기 중인 회원이 없습니다.
      </div>

      <div
        v-else
        v-for="user in confirmModal.users"
        :key="user.userId"
