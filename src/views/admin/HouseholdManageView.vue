...
const stats = computed(() => [
  ...
  { label: '입주 세대', value: householdStore.occupied, unit: '세대', desc: `전체 대비 ${occupiedRate.value}%`, descClass: 'success' },
  { label: '공실 세대', value: householdStore.empty,    unit: '세대', desc: `관리 필요 ${householdStore.needCare}세대`, descClass: 'warning' },
  { label: '이번 달 변동', value: householdStore.monthNew, unit: '건', desc: `전입 ${householdStore.moveIn} · 전출 ${householdStore.moveOut}` },
])
...
const resetFilters = () => { state.ho = ''; state.dong = ''; state.status = ''; doSearch() }
const goToPage = (page) => { state.currentPage = page; goToList() }

// ── 모달 ──
const openDetailModal = async (item) => {
  detailModal.item = item; detailModal.residents = []; detailModal.show = true; detailModal.loading = true
  try {
...
const openEditModal = () => {
  editModal.item = detailModal.item; editModal.status = detailModal.item?.status ?? '공실'
  editModal.selectedUserIds = []; editModal.error = ''; editModal.loading = false
  editModal.show = true; detailModal.show = false
}
...
const openConfirmModal = async (e, item) => {
  e.stopPropagation()
  confirmModal.item = item; confirmModal.show = true; confirmModal.users = []; confirmModal.loading = true
  try {
...
<template>
  <div class="household-page">
    ...
    <!-- 수정 모달 -->
    <BaseModal v-if="editModal.show" title="세대 정보 수정" :subtitle="'ID #' + editModal.item?.householdId + ' · ' + editModal.item?.dong + ' ' + editModal.item?.ho" @close="closeEditModal">
      <div class="form-row">
        <div class="form-group"><label class="form-label">동 (dong)</label><input class="form-input" :value="editModal.item?.dong" disabled /></div>
        <div class="form-group"><label class="form-label">호수 (ho)</label><input class="form-input" :value="editModal.item?.ho" disabled /></div>
      </div>
      <div class="form-row">
        ...
        <div class="form-group"><label class="form-label">등록 차량 수</label><input class="form-input" :value="editModal.item?.carCount != null ? editModal.item.carCount + '대' : '-'" disabled /></div>
      </div>
      ...
      <div class="form-group"><label class="form-label">등록일</label><input class="form-input" :value="editModal.item?.createdAt ?? '-'" disabled /></div>
      ...
      <template #footer>
        ...
        <button class="btn-submit" @click="submitEdit" :disabled="editModal.loading">{{ editModal.loading ? '처리 중...' : '수정 완료' }}</button>
      </template>
    </BaseModal>
...
.household-page {  display: flex; flex-direction: column; font-family: 'Noto Sans KR', sans-serif; color: #333; }
