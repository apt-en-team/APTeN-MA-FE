...
import {getParkingStats} from '@/api/parking.js'
import VueApexCharts from 'vue3-apexcharts'
import StatsCards from '@/components/admin/StatsCards.vue'

// ── 상태 ──────────────────────────────────────────────────────
const selectedTab = ref('month') // 선택된 기간 탭 (today/week/month/3months/custom)
const customStart = ref('')      // 직접 선택 - 시작일
const customEnd = ref('')      // 직접 선택 - 종료일
const statistics = ref([])      // API 응답 통계 배열
...
// ── 탭별 날짜 범위 + 집계 타입 계산 ──────────────────────────
// type: 'hourly' → x축 00~23시 / 'daily' → x축 날짜
const dateRange = computed(() => {
  ...
  switch (selectedTab.value) {
    ...
    case '3months': {
      ...
      const s = new Date(today);
      s.setMonth(today.getMonth() - 2);
      s.setDate(1)
      return {start: fmt(s), end: fmt(today), type: 'daily'}
    }
    case 'custom':
      // 직접 선택 → 사용자가 입력한 날짜 그대로, 일별 집계
...
// ── API 조회 ──────────────────────────────────────────────────
// GET /api/admin/parking/stats?startDate=&endDate=&type=
...
const statsData = computed(() => [
  {
    ...
    value: totalIn.value,
    unit: '건',
    desc: totalOut.value > 0 ? `출차 ${totalOut.value}건` : '출차 없음',
  ...
  },
  {
    ...
    desc: isHourly.value && peakHour.value != null
        ...
        : peakDate.value ? `가장 바쁜 날 ${peakDate.value}` : '-',
  },
  {
    label: '피크 타임 사용률',
    value: peakRate.value,
    unit: '%',
    // hourly면 피크 시간대, daily면 기간 내 최대 표시
    desc: isHourly.value && peakHour.value != null
        ? `${peakHour.value}시 ~ ${peakHour.value + 1}시`
        : peakRate.value > 0 ? '기간 내 최대' : '-',
...
// → categories 방식 대신 { x, y } 방식으로 막대 중앙에 레이블 맞춤
const inSeries = computed(() => {
  if (isHourly.value) {
    return Array.from({length: 24}, (_, h) => ({
      x: String(h).padStart(2, '0'),
      y: statistics.value.filter(r => r.hour === h).reduce((s, r) => s + (r.totalIn ?? r.total_in ?? 0), 0)
    }))
  }
  return chartCategories.value.map(date => ({
    x: date,
    y: statistics.value.filter(r => r.date === date).reduce((s, r) => s + (r.totalIn ?? r.total_in ?? 0), 0)
  }))
})
...
const outSeries = computed(() => {
  if (isHourly.value) {
    return Array.from({length: 24}, (_, h) => ({
      x: String(h).padStart(2, '0'),
      y: statistics.value.filter(r => r.hour === h).reduce((s, r) => s + (r.totalOut ?? r.total_out ?? 0), 0)
    }))
  }
  return chartCategories.value.map(date => ({
    x: date,
    y: statistics.value.filter(r => r.date === date).reduce((s, r) => s + (r.totalOut ?? r.total_out ?? 0), 0)
  }))
})
...
const chartOptions = computed(() => ({
  ...
  plotOptions: {bar: {borderRadius: 3, columnWidth: isHourly.value ? '65%' : '50%'}},
  colors: ['#4973E5', '#F59E0B'], // 입차: 파랑, 출차: 주황
  dataLabels: {enabled: false},
  stroke: {show: false},
  xaxis: {
    type: 'category',
    labels: {style: {fontSize: '11px', colors: '#687282'}, rotate: chartCategories.value.length > 20 ? -45 : 0},
  ...
  yaxis: {
    ...
    labels: {
      ...
      formatter: val => Number.isInteger(val) ? String(val) : '', // 정수 눈금만 표시
    },
  },
  grid: {borderColor: '#F1F3F5', strokeDashArray: 4, yaxis: {lines: {show: true}}, xaxis: {lines: {show: false}}},
  legend: {
    show: true,
    position: 'top',
  ...
  tooltip: {
    ...
    custom: ({series, seriesIndex, dataPointIndex, w}) => {
      ...
      return `<div style="background:#fff;border:1.5px solid #E2E8F0;border-radius:8px;padding:10px 14px;font-size:13px;font-family:Noto Sans KR,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,0.07);min-width:120px;">
    <div style="font-size:11px;color:#687282;margin-bottom:8px;font-weight:600;">${isHourly.value ? cat + '시' : cat}</div>
  ...
  <div class="parking-stats-page">
    ...
    <div class="chart-card">
      ...
      <div class="chart-section">
        ...
        <!-- 데이터 없음 -->
        <div v-else-if="statistics.length === 0" class="chart-empty">해당 기간의 데이터가 없습니다.</div>

        <!-- ApexCharts 막대 차트
             isHourly: true  → x축 00~23시간 (오늘/이번달)
             isHourly: false → x축 날짜 (이번주/최근3개월/직접선택) -->
        <VueApexCharts v-else type="bar" height="340" :options="chartOptions" :series="chartSeries"/>
      </div>
    </div>
  </div>
</template>
