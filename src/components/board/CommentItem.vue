...
<template>
  <div class="comment-item" v-if="comment.isDeleted !== 1">
    ...
    <div v-if="editing" class="comment-edit-wrap">
      <input v-model="editingContent" class="comment-edit-input" :class="mode === 'admin' ? 'input-admin' : 'input-resident'"/>
      <div class="comment-edit-btns">
        <button class="btn-edit-cancel" @click="cancelEdit">취소</button>
        <button
          class="btn-edit-save"
          :class="mode === 'admin' ? 'save-admin' : 'save-resident'"
          @click="saveEdit"
        >저장</button>
      </div>
    </div>
...
.comment-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; flex-wrap: nowrap; gap: 6px; }
...
.comment-date { font-size: 11px; color: #bbb; margin-bottom: 6px; display: block; }
.btn-edit-comment {
...
.comment-content { font-size: 13px; color: #444; line-height: 1.5; margin: 10px 0; }
...
.input-resident:focus { border-color: #4973E5; }
.input-admin:focus    { border-color: #2B3A55; }
.comment-edit-btns { display: flex; gap: 6px; justify-content: flex-end; }

.save-resident { background: #4973E5; }
.save-resident:hover { background: #3860CC; }
.save-admin    { background: #2B3A55; }
.save-admin:hover    { background: #1E2A3E; }
.btn-edit-cancel {
