...
const currentTitle = computed(() => route.name || '대시보드')
...
.sidebar {
  width: 240px; background: #fff; border-right: 1px solid #E8EBF0;
  display: flex; flex-direction: column; flex-shrink: 0;
}
.sidebar-logo {
  padding: 20px; border-bottom: 1px solid #F0F1F5;
  display: flex; align-items: center; gap: 10px;
}
.logo-icon {
  width: 36px; height: 36px; border-radius: 10px;
  background: #4F6EF7; color: #fff; font-size: 16px; font-weight: bold;
...
.nav-item {
  ...
  font-size: 13px; color: #666; text-decoration: none; margin-bottom: 2px;
}
.nav-item:hover { background: #F3F5F9; color: #333; }
.nav-item.router-link-active {
  background: #EEF1FF; color: #4F6EF7; font-weight: 600;
}
.sidebar-profile {
  padding: 16px; border-top: 1px solid #F0F1F5;
  display: flex; align-items: center; gap: 10px;
}
.profile-avatar {
  width: 34px; height: 34px; border-radius: 50%; background: #E8EBF0;
  display: flex; align-items: center; justify-content: center;
  font-size: 13px; font-weight: 600; color: #777;
}
.profile-name { font-size: 13px; font-weight: 600; color: #333; }
.profile-info { font-size: 11px; color: #999; }
