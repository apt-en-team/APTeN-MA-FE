  ...
  <div class="vehicle-list-view">
    ...
    <div class="table-section">
      ...
      <AdminTable :columns="columns" :rows="state.list">
        ...
        <template #action="{ row }">
          <button class="btn-detail" @click.stop="openConfirmModal(row)">상세</button>
        </template>
    ...
    <BaseModal
      ...
      title="차량 상세 정보"
      :subtitle="'ID #' + confirmModal.vehicle?.vehicleId"
      @close="closeConfirmModal"
    >
      <!-- 히어로 -->
      <div class="detail-hero">
        <span :class="['detail-status-badge', statusClass(confirmModal.vehicle?.status)]">
          {{ statusLabel(confirmModal.vehicle?.status) }}
        </span>
        <h2 class="detail-title">{{ confirmModal.vehicle?.licensePlate }}</h2>
        <p class="detail-sub">{{ confirmModal.vehicle?.carModel ?? '-' }} · 차량 정보</p>
      </div>
      ...
      <div class="detail-grid">
        <div class="detail-cell">
          ...
          <span class="detail-cell-value">#{{ confirmModal.vehicle?.vehicleId }}</span>
        </div>
        <div class="detail-cell">
          <span class="detail-cell-label">차량번호</span>
          <span class="detail-cell-value">{{ confirmModal.vehicle?.licensePlate }}</span>
        </div>
        <div class="detail-cell">
          <span class="detail-cell-label">차종</span>
          <span class="detail-cell-value">{{ confirmModal.vehicle?.carModel ?? '-' }}</span>
        </div>
        <div class="detail-cell">
          <span class="detail-cell-label">등록일</span>
          <span class="detail-cell-value">{{ confirmModal.vehicle?.createdAt ?? '-' }}</span>
        </div>
        <div class="detail-cell">
        ...
        <div class="detail-cell">
          ...
          <span class="detail-cell-value">{{ confirmModal.vehicle?.approvedAt ?? '-' }}</span>
        </div>
      </div>

      <template #footer>
        <button class="btn-cancel" @click="closeConfirmModal">닫기</button>
        <template v-if="confirmModal.vehicle?.status === 'PENDING'">
          <button
            class="btn-submit-reject"
            @click="handleConfirm('reject')"
            :disabled="confirmModal.loading"
          >
            {{ confirmModal.loading ? '처리 중...' : '거부' }}
          </button>
          <button
...
.btn-submit-reject { padding: 9px 24px; background: #FEE2E2; color: #E53E3E; border: 1px solid #FECACA; border-radius: 7px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: 'Noto Sans KR', sans-serif; }
.btn-submit-reject:hover:not(:disabled) { background: #fecaca; }
